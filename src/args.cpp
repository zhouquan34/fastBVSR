...
//	cout << "-m: matrix input file with rows representing subjects" << endl;
...
void print_manual(void){
	...
	cout << "--pi-a2:     group-A prior alpha of pi" << endl;
	cout << "--pi-b2:     group-A prior beta of pi" << endl;
	cout << "--pi-min:    min pi" << endl;
	cout << "--pi-max:    max pi" << endl;
	cout << "--pi2-min:   group-A min pi" << endl;
	cout << "--pi2-max:   group-A max pi" << endl;
	...
	cout << "--size-a:    the expected number of group-A covariates to be included" << endl;
	cout << "--sbf-p:     number of permutations for estimating sBF" << endl;
	cout << "--sbf:       use sBF instead of BF" << endl;
...
void init_paras(void){
	...
	g_paras["--pi-a2"] = PARA_PI_A2;
	g_paras["--pi-b2"] = PARA_PI_B2;
	g_paras["--pi-min"] = PARA_MINPI; 
	g_paras["--pi-max"] = PARA_MAXPI; 
	g_paras["--pi2-min"] = PARA_MINPI2; 
	g_paras["--pi2-max"] = PARA_MAXPI2; 
	...
	g_paras["--size-a"] = PARA_SIZEA;
	g_paras["--max-size"] = PARA_MAXS;
	g_paras["--min-size"] = PARA_MINS;
	g_paras["--sbf"] = PARA_SBF;
	g_paras["--sbf-p"] = PARA_SBFP;
...
int set_args_part_two (string para, char* val){
	...
	switch(g_paras[para]){
		...
		case PARA_MINPI2:
			g_min_pi_2 = atof(val); g_pi_range_set_2 = 1; return 1;
		case PARA_MAXPI2:
			g_max_pi_2 = atof(val); g_pi_range_set_2 = 1; return 1;	
		case PARA_MAXJ:
		...
		case PARA_PI_A2:
			g_pi_alpha_2 = atoi(val); return 1;
		case PARA_PI_B2:
			g_pi_beta_2 = atoi(val); return 1;	
		case PARA_PREC:
			g_precomp = atoi(val); return 1;
		case PARA_SBF:
			g_sbf = 1; return 1;
		case PARA_SBFP:
			g_sbf = 1; g_sbf_perm = atoi(val); return 1;
		case PARA_SIZEA:
			g_expect_size_a = atoi(val); return 1;
		case PARA_MAXS:
...
//	string out_prefix_copy = g_out_prefix;
