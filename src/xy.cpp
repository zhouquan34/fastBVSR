...
void X_init(const vector<int>& s, double* X){
	record_time();
	int col = s.size() + g_n_cov;
	if (g_n_cov > 0){
		copy1D(X, g_cov, g_n_sub * col);
	}
	for (int i=g_n_cov; i<col; i++){
		int k = s[i - g_n_cov];
		copy1D(&X[i * g_n_sub], &g_data[k * g_n_sub], g_n_sub);
	}
//	cout << "X_init" << endl;  print_array(X, 5);
	record_time("UpdateX");
	return;
}
...
void X_add (const vector<int>& add, int p_last, double* X_last, double* X){
	record_time();
	int s = add.size();
	copy1D(X, X_last, g_n_sub * p_last);
	for (int i=0; i < s; i++){
		int k = add[i]; 
		copy1D(&X[(i + p_last) * g_n_sub], &g_data[k * g_n_sub], g_n_sub);
	}
//	cout << "X_add" << endl;  print_array(X, 5);
	record_time("UpdateX");
	return;
}
...
void X_del (const vector<int>& del, int p_last, double* X_last, double* X){
	record_time();
	int s = del.size();
	int c = 0;
	int d = 0;
	for (int i=0; i< s; i++){
		...
		if (r - c > 0){
			copy1D(&X[d * g_n_sub], &X_last[c * g_n_sub], (r-c) * g_n_sub);
		}
		d = d + r - c;
		c = r + 1;		
	}
	if (c < p_last){	
		copy1D(&X[d * g_n_sub], &X_last[c * g_n_sub], (p_last - c) * g_n_sub);
	}
//	cout << "X_del" << endl;  print_array(X, 5);
	record_time("UpdateX");
	return;
}
...
void Xy_init (double* X, int p, double* Xy){
//	record_time();
...
void XX_init (double* X, int p, double* XX){
	record_time();
	int c = 0;
	for (int i=0; i<p; i++){
		for (int j=0; j<i; j++){
...
void XX_init (const vector<int>& subset, double* X, int p, double* XX){
	record_time();
	int c = 0;
	for (int i=0; i<g_n_cov; i++){
		for (int j=0; j<=i; j++){
			XX[c] = g_xtx[(g_precomp + g_n_cov) * i + j];
			c ++;
		}
	}
...
void XX_add (const vector<int>& add, int p_last, double* X, double* XX_last, double* XX){
	record_time();
	int s = add.size();
	int c = p_last * (p_last + 1) / 2;
	copy1D(XX, XX_last, c);
	for (int i=0; i<s; i++){	
		mat_vec_mul(X, &X[(p_last + i)*g_n_sub], p_last + i, g_n_sub, &XX[c], 0);	
		c += (p_last + i);
...
void XX_add (const vector<int>& subset, const vector<int>& add, int p_last, double* X, double* XX_last, double* XX){
	record_time();
	int s = add.size();
	int c = p_last * (p_last + 1) / 2;
	copy1D(XX, XX_last, c);
		...
		if (o < g_precomp){
			...
			for (int j = g_n_cov; j < p_last + i; j++){
				if (g_single_ord[subset[j - g_n_cov]] < g_precomp){
					...
					XX[c] = vec_vec_mul(&X[(p_last + i) * g_n_sub], &X[j * g_n_sub], g_n_sub);
				}
				c ++;
			}
		}else{
...
//		cerr << c << "\t" << p << "\t" << p_last << "\t" << nxx << "\t" << to_del.size() << endl;
//		print_array(XX_last, nxx);
//		print_array(XX, c);
	}
	
//	cout << "XX_del" << endl;  print_array(XX, 5);
	record_time("UpdateXX");
	return;
}
