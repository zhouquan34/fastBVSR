// R is upper triangle
// diags^2 = (0, 0, .. , 0, s1, s2, .. , s1, s2, ..., s1, s1, s2, s2)
#include "chol.h"
...
/*
void Chol_add (int add, int p, double* XX, double* R, double* next_R, double* diags){
	record_time();
	...
	if (diags == NULL){
		...
		for (i = 0; i < add; i ++){
			forward_sub(p + i, p + add,  XX, next_R, diags[i] * diags[i]);			
		}
	}
...
void Chol_add (int add, int p, double* XX, double* R, double* next_R){
	record_time();
	int next_p = p + add;
//	memset(next_R, 0,  next_p * next_p * sizeof(double));
...
//	cout << "Chol_add" << endl;  print_array(next_R, 5);
	record_time("CholAdd");
	return;	
}


void Chol_add_no_time (int add, int p, double* XX, double* R, double* next_R, double* diags = NULL){
	int next_p = p + add;
//	memset(next_R, 0,  next_p * next_p * sizeof(double));
...
//	cout << "Chol_del" << endl;  print_array(next_R, 5);
	record_time("CholDel");
	return;
}
...
double Chol_decomp_A (int p, double* XX, double* R, double* diags){
	record_time();
	if (g_n_cov == 0){
		double* R0 = allocate1D(1); 
		R0[0] = sqrt(XX[0] + diags[0] * diags[0]);	
		Chol_add_no_time(p - 1, 1, XX, R0, R, &diags[1]);
		free1D(R0);
	}else{
		Chol_add_no_time(p - g_n_cov, g_n_cov, XX, g_cov_R, R, &diags[g_n_cov]);	
	}
	double log_det = 0.0;
	for (int i=0; i<p; i++){
		log_det += log(R[i * p + i]);
